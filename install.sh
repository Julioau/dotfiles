...
link_with_privilege() {
    ...
    if [ "$DRY_RUN" = true ]; then
        log_dry "Would use pkexec to link $abs_source -> $dest"
        return
    fi
    
    # We use a single pkexec call per file for clarity, or we could batch them if passed as array
    # But adhering to 'simple arguments', we do one at a time.
    log_warn "Need root privileges to link $name to $dest."
    pkexec bash -c "ln -sf '$abs_source' '$dest'"
