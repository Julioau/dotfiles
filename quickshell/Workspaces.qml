...
RowLayout { // The root element, arranges items horizontally.
    ...
    function getIcon(clientData) {
        if (!clientData) return ""; // If no client data, return an empty string.
        var cls = clientData.class; // Get the class name of the client/window.
        
        // 1. Icon Theme: Check if an icon exists in the Quickshell icon theme for the class.
        if (Quickshell.iconPath(cls, true) !== "") return cls; // If found, use the class name as the source.
        
        // 2. Desktop Entry: Search through installed desktop applications.
        // This is useful for matching icons for applications based on their StartupWMClass.
        var apps = DesktopEntries.applications.values; // Get all available desktop entries.
        for (var i = 0; i < apps.length; i++) { // Loop through each application.
            // If the application's startup class matches the client's class and it has an icon, return its icon.
            if (apps[i].startupClass === cls && apps[i].icon) {
                return apps[i].icon;
            }
        }
        
        // // 3. Desktop Entry: By ID (Exact match) - This part is commented out but shows an alternative.
        // var byId = DesktopEntries.byId(cls);
        // if (byId && byId.icon) return byId.icon;

        return ""; // If no suitable icon is found, return an empty string.
    }
    ...
    Repeater {
        ...
        delegate: Rectangle { // Each workspace is represented by a Rectangle.
            ...
            property var monitor: workspace.monitor || (Hyprland.monitors.count > 0 ? Hyprland.monitors.values[0] : null)
            ...
            // It tries to make empty workspaces square, and occupied ones match monitor aspect ratio.
                ...
                property real monitorRatio: monitor ? (monitor.width / monitor.height) : (16/9)
                ...
                border.color: workspace.urgent ? Theme.red : (wsMouse.containsMouse ? Theme.green : (workspace.active ? Theme.blue : Theme.transparent)) // Red if urgent, Green if hovered, Blue if active.
                ...
                Repeater {
                    ...
                    delegate: Rectangle { // Each window is represented by a Rectangle.
                        ...
                        property real monW: mon ? mon.width : 1920 // Default width if monitor is null.
                        property real monH: mon ? mon.height : 1080 // Default height if monitor is null.
